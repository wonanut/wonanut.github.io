...
function _init_toolbar()
{
	...
	simulate.fillText("MAHATTAN", 8, 25);

	modify.font = "16px Arial";
	modify.fillStyle = "#fff";
	modify.fillText("SIMULATE", 10, 25);
}
...
function clk_simulate()
{
	if(mode == 1)
	{
		updateTip('You can not get the Manhattan distance since it is in the stimulate mode currently, try in edit mode.');
		return;
	}
...
function clk_modify()
{
	...
	{
		updateTip('You can not enter simulation mode without setting the start point or end point.');
		return;
	}
	...
	if(mode == 0)
	{
		...
		updateTip('Now in the simulate mode, push unit: (' + startX + ',' + startY + ') into the openList');
	}
	else
	{
		mode = 0;
